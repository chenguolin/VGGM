...
# Modifications:
    ...
    ## 2. Remove all inplace operations to avoid potential autograd issues
...
class MemBlock(nn.Module):
    def __init__(self, n_in, n_out):
        super().__init__()
        self.conv = nn.Sequential(conv(n_in * 2, n_out), nn.ReLU(),
                                  conv(n_out, n_out), nn.ReLU(), conv(n_out, n_out))
            ...
            if i == len(model):
                ...
                if isinstance(b, MemBlock):
                    ...
                    if mem[i] is None:
                        ...
                        mem[i] = xt.clone()  # for gradient safety
...
class TAEHV(nn.Module):
    ...
    def __init__(self, checkpoint_path="taehv.pth", decoder_time_upscale=(True, True), decoder_space_upscale=(True, True, True)):
        ...
        self.encoder = nn.Sequential(
            conv(TAEHV.image_channels, 64), nn.ReLU(),
        ...
        self.decoder = nn.Sequential(
            Clamp(), conv(TAEHV.latent_channels, n_f[0]), nn.ReLU(),
            ...
            nn.ReLU(), conv(n_f[3], TAEHV.image_channels),
        )
