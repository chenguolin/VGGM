...
from src.models import Wan, MyEMAModel, get_optimizer, get_lr_scheduler
import src.utils.util as util
import src.utils.vis_util as vis_util


def main():
    PROJECT_NAME = "ODEPairs"

    parser = argparse.ArgumentParser(
        description="Fine-tuning Wan to align with the ODE pairs."
    )
    ...
    logger.info(f"Load [{len(train_dataset)}] training samples\n")
    ...
    if args.load_pretrained_model is not None:
        ...
        if os.path.exists(pretrained_ema_path):
            ...
            _ema_states.copy_to(model.parameters())
            del _ema_states
    ...
    for epoch in range(configs["train"]["epochs"]):
        ...
        for batch in train_loader:
            ...
            with accelerator.accumulate(model):
                ...
                outputs = model(batch, func_name="compute_loss_ode", dtype=weight_dtype, vae=vae)
            ...
            if accelerator.sync_gradients:
                ...
                if args.use_ema:
                    if args.ema_on_cpu:
                        ema_states.step([p.cpu() for p in model.parameters()])
                    else:
